...
public class FlashSelenium implements IFlashSelenium {
	...
	static String createJSPrefix_Fixed(String flashObjectId) {
		return "window.document.getElementById('" + flashObjectId + "')";
	}	

	private void createJSPrefix() {
		if (flashJSStringPrefix == null) {
			String[] prefixesToTest = new String[] {
					createJSPrefix_Fixed(flashObjectId),
					createJSPrefix_window_document(flashObjectId),
					createJSPrefix_document(flashObjectId)
			};
			int i = 0;
			while (flashJSStringPrefix == null && i < prefixesToTest.length) {
				flashJSStringPrefix = prefixesToTest[i];
			}
			if (flashJSStringPrefix == null)
				throw new RuntimeException("Impossible to get the Flash string prefix! Try changing the browser");
			else
				flashJSStringPrefix += ".";
		}
	}
